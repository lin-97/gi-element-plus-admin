...
function storeSetup() {
   ...
   * 判断是否为首页
   */
  const isHomePath = (path: string) => path === '/' || path === HOME_PATH
  ...
  const close = (type: string, path?: string) => {
    ...
    // 关闭当前
    if (type === 'current') {
      if (targetIndex < 0 || tabList.value[targetIndex].meta?.affix)
        return
      removeTabByIndex(targetIndex)
      if (currentPath === targetPath) {
        const lastTab = tabList.value.at(-1)
        if (lastTab)
          jumpToTab(lastTab.path)
      }
      return
    }
   ...
   */
  const init = () => {
    if (tabList.value.some(i => !i?.meta?.affix))
      return
    reset()
  }
